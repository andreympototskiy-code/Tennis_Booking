...
# -*- coding: utf-8 -*-
"""
Улучшенный модуль автоматического бронирования теннисных кортов
Использует правильный API /initialize для получения данных
"""
...
class EnhancedAutoBooking:
    ...
    def get_initialize_data(self, date: str) -> Optional[Dict]:
        """Получение данных инициализации через API"""
        ...
        try:
            ...
            # Проверяем, что это JavaScript код с данными
            if 'window.$INITIAL' in response.text:
                ...
                if json_match:
                    ...
                    logger.info("Данные инициализации успешно извлечены")
                    return data
            else:
                logger.error("Данные инициализации не найдены в ответе")
    ...
    def find_available_court(self, date: str, court_type: str = 'ground', 
        ...
        init_data = self.get_initialize_data(date)
        if not init_data:
            logger.error("Не удалось получить данные инициализации")
            return None
        
        # Извлекаем корты из данных
        court_types = init_data.get('instructions', {}).get('set', {}).get('court_types', [])
        time_blocked = init_data.get('instructions', {}).get('set', {}).get('time_blocked', [])
        
        # Ищем грунтовые корты
        ground_courts = []
        for court_type_data in court_types:
            if court_type_data.get('label') == 'grunt':  # Грунт
                ground_courts.extend(court_type_data.get('courts', []))
...
def main():
    """Тестирование улучшенного модуля бронирования"""
    booking = EnhancedAutoBooking()
    
    # Тестируем на завтра
    tomorrow = (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')
    
    print(f"🎾 Тестирование улучшенного автоматического бронирования на {tomorrow}")
    print("=" * 70)
